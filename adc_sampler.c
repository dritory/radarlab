...
#define OUTPUT_DATA "./out/adcData.bin" // path and filename to dump buffered ADC data
...
int main(int argc, char *argv[])
{
	...
	long num_samples = DEFAULT_NUM_SAMPLES;
	...
	char filename[128];
	const char* pretext = "./out/";
	const char* extension = ".bin";

	if ( argc > 2 ) {
		strcpy(filename, pretext);

		char* file = argv[2];

		if ( sizeof(filename) < strlen(file) + strlen(pretext) + 1 ) { /* +1 is for null character */
			fprintf(stderr, "Name '%s' is too long\n", file);
			return EXIT_FAILURE;
		}
		strcat(filename, file );

		if ( sizeof(filename) < (strlen(filename) + strlen(extension) + strlen(pretext) + 1) ) {
    ...
    FILE *adc_data_file = fopen(filename, "wb+");
